...
|---|---|---|---|
| Contract | `tests/contract/` | Yes | One file per source module — each of the twenty-five tools, plus `config`, `errors`, the cross-tool `symlink` policy, and every `src/lib/` primitive (`atomic`, `binary`, `ignore`, `match-cascade`, `output`, `paths`, `render-lines`, `rg`, `text`, `textfile`, `log`, `treesitter`, `syntax-annotate`): the input/output/error contract of that module, driven through `dispatch` where it has a tool surface and called directly where it is a primitive. |
