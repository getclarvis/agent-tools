...
export const copy: ToolDef = {
  ...
  description:
    ...
    "rejected; use bash for directory copies. The source's permission mode is preserved. Refuses " +
  ...
  async handler(args, config) {
    ...
    return withFileLocks([absSrc, absDst], async () => {
      ...
      const tmp = path.join(dstDir, `.clarvis-tmp-${uniqueToken()}`);
      try {
        await fs.mkdir(dstDir, { recursive: true });
        await fs.copyFile(absSrc, tmp);
        await fs.chmod(tmp, srcStat.mode & 0o777);
        await fs.rename(tmp, absDst);
      } catch (err) {
