...
`isError: true` and puts this JSON in `text`. Success never uses the envelope — its `text` is the
tool's output (or, for `bash`, a `{ exit_code, … }` JSON object, which is **not** an error even on a
non-zero exit).
